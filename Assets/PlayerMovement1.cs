...
public class PlayerMovement1 : MonoBehaviour       // PS4 CONTROLLER
{
    ...
    void FixedUpdate()
    {
        if (canMove)
        {
            ...
            if (IsGrounded())
            {
                ...
                // Jump
                if (IsGrounded() && Input.GetKey("joystick button 1"))
                {
    ...
    private void Update()
    {
        float h = Input.GetAxis("JoystickLeftX"); // Custom axis name
        float v = Input.GetAxis("JoystickLeftY");
